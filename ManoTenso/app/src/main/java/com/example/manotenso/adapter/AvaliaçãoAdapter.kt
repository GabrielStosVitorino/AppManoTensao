...
class AvaliaçãoAdapter(
    val listaAvaliacao: MutableList<FiltroPorAvaliacao>,
    private val onclick: (id: Int) -> Unit
) : RecyclerView.Adapter<AvaliaçãoAdapter.AvaliacaoPrestadorViewHolder>() {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): AvaliaçãoAdapter.AvaliacaoPrestadorViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(
            R.layout.prestador_item,
            parent, false
        )
        return AvaliacaoPrestadorViewHolder(itemView)
    }

    override fun onBindViewHolder(holder: AvaliaçãoAdapter.AvaliacaoPrestadorViewHolder, position: Int) {
        (holder as AvaliacaoPrestadorViewHolder).vincular(listaAvaliacao[position])
    }
    ...
    inner class AvaliacaoPrestadorViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
